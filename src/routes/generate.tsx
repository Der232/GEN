...
function GeneratePage() {
	...
	const [countOption, setCountOption] = useState<"5" | "10" | "15" | "custom">(
		"5",
	);
	...
	const startUpload = (file: File) => {
		...
		if (!type) {
			setFileError(
				"Unsupported file type. Only PDF (.pdf), PowerPoint (.pptx), and Word (.docx) are supported.",
			);
			return;
		}
				...
				xhr.upload.onprogress = (e) => {
					if (e.lengthComputable) {
						...
						if (elapsedSec > 0.2) {
							...
							const sec =
								speedBytesPerSec > 0
									? Math.max(1, Math.round(remainingBytes / speedBytesPerSec))
									: 0;
	...
	const handleGenerate = async (e: React.FormEvent) => {
		...
		const rawCount =
			countOption === "custom" ? customCount : Number(countOption);
		if (rawCount > 30) {
	...
	const handleSaveToLibrary = async () => {
		...
		try {
			...
			if (!saveRes.ok)
				throw new Error(saveData.error || "Failed to save exam.");
	...
	const handleStartPractice = async (
		...
		try {
			...
			if (!attRes.ok)
				throw new Error(attData.error || "Failed to start attempt.");
		...
		<div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10 animate-fade-in">
			...
			<div className="mb-5 sm:mb-8">
				...
				<p className="text-xs sm:text-sm text-[var(--text-secondary)] mt-1">
					Specify a topic, target difficulty, and question format. AI will build
					an exam with verified answer keys.
				</p>
			</div>
			...
			{!result ? (
				...
				<form
					...
					className="gen-card p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6"
				>
					{/* Topic Input */}
					<div>
							...
							{subjectOption === "Custom Subject / Category..." ? (
								<div className="mt-2.5">
									...
									<p className="text-[11px] text-[var(--text-muted)] mt-1">
										AI will understand and tailor the questions to this custom
										subject context.
									</p>
								</div>
							) : subjectOption === "Auto-Detect (AI)" ? (
								<p className="text-[11px] text-[var(--text-muted)] mt-1.5">
									AI will auto-detect the subject discipline directly from your
									topic details.
								</p>
							) : null}
						</div>
                ...
                `}
							...
							<div className="mt-2.5 space-y-1">
								<div className="flex flex-wrap items-center gap-2 sm:gap-3">
									<span className="text-xs text-[var(--text-secondary)]">
										Custom question count (1 – 30 max):
									</span>
									<input
										type="number"
										min={1}
                    ...
                    `}
						...
						{questionTypes.length > 1 && (
							<p className="text-[11px] text-[var(--text-muted)] mt-1.5">
								Both formats selected: questions will be a randomized mix of
						...
						{isRegenerating && (
							<div className="absolute inset-0 z-20 backdrop-blur-md bg-[var(--bg-main)]/70 rounded-2xl flex flex-col items-center justify-center p-6 text-center border border-[var(--border-subtle)] animate-fade-in shadow-xl">
								...
								<p className="text-xs text-[var(--text-secondary)] mt-1 max-w-sm">
									Synthesizing fresh questions and verified answer keys based on
									your material.
								</p>
							</div>
						)}
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between px-1">
								...
								<span className="text-[11px] text-[var(--text-muted)] flex items-center gap-1.5">
									...
									<span>
										Choices & answer keys hidden for authentic practice
									</span>
								</span>
							</div>
							...
							{result.questions.map((q, idx) => (
								<div
									key={q.order ? `q-${q.order}` : `q-${idx}`}
									className="gen-card p-4 sm:p-5 space-y-2"
								>
