...
function ResultsPage() {
	...
	const recordAttemptCompletion = useExamPerformanceStore(
		(state) => state.recordAttemptCompletion,
	);
	const recordQuestionAnswer = useExamPerformanceStore(
		(state) => state.recordQuestionAnswer,
	);
	const questionPerformance = useExamPerformanceStore(
	...
	const [retryStates, setRetryStates] = useState<
		Record<string, QuestionRetryState>
	>({});
	...
	const handleCheckRetryAnswer = (q: Question) => {
		...
		const isCorrect =
			state.selectedAnswer.trim().toLowerCase() ===
		...
		return (
			<div className="flex flex-col items-center justify-center min-h-[60vh] max-w-md mx-auto text-center px-4">
				...
				<p className="text-xs text-[var(--text-muted)] mb-6">
					This exam attempt could not be found or you do not have permission to
					view it.
				</p>
				<Link
					to="/my-exams"
			...
			<div className="gen-card p-4 sm:p-8 text-center relative overflow-hidden">
				<div className="max-w-md mx-auto">
					...
					<div className="flex items-center justify-center gap-2 mb-1">
						...
						<span className="text-xs text-[var(--text-muted)] truncate max-w-[200px] sm:max-w-none">
							{examTitle}
						</span>
					</div>
				...
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
					<div>
						...
						<p className="text-[11px] text-[var(--text-muted)] mt-0.5">
							Review full answers, conceptual breakdowns, and ask the AI tutor
							for step-by-step proofs.
						</p>
					</div>
									...
									<div className="flex items-start justify-between gap-3">
										<div className="flex items-center gap-2 flex-wrap">
											...
											{questionPerformance[q.id] &&
												questionPerformance[q.id].timesAttempted > 1 && (
													<span className="text-[10px] text-[var(--text-muted)] font-medium">
														• Attempted{" "}
														{questionPerformance[q.id].timesAttempted}x (
														{questionPerformance[q.id].timesCorrect} correct)
													</span>
												)}
										</div>
										...
										<div className="p-4 rounded-xl bg-[var(--bg-surface-elevated)] border border-[var(--border-strong)] space-y-3 animate-fade-in">
											...
											{q.options && q.options.length > 0 && (
												<div className="grid gap-2">
													{q.options.map((opt) => {
														const isSelected =
															retryStates[q.id]?.selectedAnswer === opt;
														return (
															<button
																key={opt}
																type="button"
																onClick={() =>
																	handleSelectRetryAnswer(q.id, opt)
																}
													...
													{["True", "False"].map((val) => {
														...
														return (
															<button
																...
																type="button"
																onClick={() =>
																	handleSelectRetryAnswer(q.id, val)
																}
											...
											{q.type === "short-answer" && (
												<input
													...
													onChange={(e) =>
														handleSelectRetryAnswer(q.id, e.target.value)
													}
													...
													<div className="flex items-center gap-1.5">
														{retryStates[q.id]?.isCorrect ? (
															...
															<span className="px-2.5 py-1 rounded-md badge-danger text-xs font-semibold flex items-center gap-1">
																<XCircle className="h-3.5 w-3.5" /> Still
																incorrect. Try again!
															</span>
														)}
													</div>
												...
												<div className="flex flex-wrap gap-1.5 pt-1">
													<button
														...
														onClick={() =>
															handleRequestExplanation(
																q,
																"Can you break down the mathematical/logical proof in simple terms?",
															)
														}
														disabled={isExplaining}
													...
													<button
														...
														onClick={() =>
															handleRequestExplanation(
																q,
																"Give me a mnemonic or memory trick to never forget this.",
															)
														}
														disabled={isExplaining}
														className="px-2.5 py-1 rounded-lg text-[10px] bg-[var(--bg-surface)] border border-[var(--border-subtle)] hover:border-[var(--border-strong)] hover:text-[var(--text-primary)] text-[var(--text-secondary)] cursor-pointer transition"
