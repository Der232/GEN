...
import { db } from '#/db'
import { exams, questions, attempts, attemptAnswers, user, session, account } from '#/db/schema'
      ...
      POST: async ({ request }) => {
        try {
          ...
          for (const att of userAttempts) {
            await db.delete(attemptAnswers).where(eq(attemptAnswers.attemptId, att.id))
            await db.delete(attempts).where(eq(attempts.id, att.id))
          }

          // 3. Delete auth records (sessions, accounts, user row)
