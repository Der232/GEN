			...
			<div className="mb-5 sm:mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
				<div>
					...
					<p className="text-xs sm:text-sm text-[var(--text-secondary)] mt-1">
						Browse and practice exams created by students and educators across
						subjects.
					</p>
				</div>
				...
				<Link
					to="/generate"
					className="btn-primary self-start text-xs no-underline px-3.5 sm:px-4 py-1.5 sm:py-2"
				>
				...
				<div className="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3">
					...
					<div className="relative flex-1 w-full">
						...
						<input
							...
							className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] pl-10 pr-4 py-2 sm:py-2.5 text-xs sm:text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--border-strong)] transition"
						/>
					</div>
					...
					<div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
						...
						<select
							...
							className="w-full sm:w-auto rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] px-3 py-2 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--border-strong)]"
						>
				...
				<div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-xs">
					{SUBJECT_FILTERS.map((subj) => (
						<button
							...
							className={`
                px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg whitespace-nowrap transition-all text-xs font-medium cursor-pointer border
                ${
			...
			{loading ? (
				<div className="py-20 flex flex-col items-center justify-center text-center">
					...
					<p className="text-xs text-[var(--text-secondary)]">
						Loading community exams...
					</p>
				</div>
			) : filteredExams.length === 0 ? (
				<div className="gen-card p-8 sm:p-12 text-center max-w-md mx-auto my-6 sm:my-8">
					...
					<Link
						to="/generate"
						className="btn-primary text-xs inline-flex items-center gap-1.5 no-underline"
					>
				...
				<div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4">
					{filteredExams.map((ex) => (
						<div
							...
							className="gen-card gen-card-hover p-3 sm:p-5 flex flex-col justify-between group cursor-pointer transition-all"
						>
							<div>
								...
								<h3 className="font-heading text-xs sm:text-base font-bold text-[var(--text-primary)] mb-1 sm:mb-1.5 line-clamp-2 group-hover:underline leading-snug">
									{ex.title}
								</h3>

								<p className="text-[10px] sm:text-xs text-[var(--text-secondary)] line-clamp-2 mb-2.5 sm:mb-4 leading-relaxed">
									{ex.description ||
										"Comprehensive test questions covering core concepts and problem solving."}
								</p>
							</div>

							<div className="pt-2 sm:pt-3 border-t border-[var(--border-subtle)] flex items-center justify-between gap-1">
								<div className="flex items-center gap-1 text-[10px] sm:text-[11px] text-[var(--text-muted)] min-w-0">
									<User className="h-2.5 w-2.5 sm:h-3 sm:w-3 shrink-0" />
									<span className="truncate max-w-[55px] sm:max-w-[110px]">
										{ex.authorDisplayName}
									</span>
									<span>·</span>
									<span className="shrink-0">{ex.questionCount}Q</span>
								</div>
