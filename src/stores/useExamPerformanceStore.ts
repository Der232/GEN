...
interface ExamPerformanceState {
	...
	getQuestionPerformance: (
		questionId: string,
	) => QuestionPerformanceRecord | undefined;
		...
		(set, get) => ({
			...
			recordQuestionAnswer: ({ questionId, examId, userAnswer, isCorrect }) => {
				set((state) => {
					...
					const timesCorrect =
						(prevRecord?.timesCorrect || 0) + (isCorrect ? 1 : 0);
					...
					return {
						...
						totalQuestionsCorrect:
							state.totalQuestionsCorrect + (isCorrect ? 1 : 0),
						questionPerformance: {
			...
			recordAttemptCompletion: ({ examId, score, totalQuestions, answers }) => {
				set((state) => {
					...
					if (answers && Array.isArray(answers)) {
						for (const ans of answers) {
							...
							const timesCorrect =
								(prevQ?.timesCorrect || 0) + (ans.isCorrect ? 1 : 0);
			...
			syncWithServer: (serverStats) => {
				set((state) => ({
					completedExamAttempts: Math.max(
						state.completedExamAttempts,
						serverStats.examsTaken,
					),
				}));
			},
