...
export async function checkUploadLimit(userId: string): Promise<{
	...
	try {
		...
		const dailyDocs = await db
			...
			.from(documents)
			.where(
				and(eq(documents.userId, userId), gte(documents.createdAt, oneDayAgo)),
			);
...
export async function checkGenerationLimit({
	...
	try {
		...
		const cooldownCutoff = new Date(
			now - LIMITS.GENERATION_COOLDOWN_SECONDS * 1000,
		);
...
/**
 * Check if a user or IP can request an AI explanation.
 * Enforces a maximum of 20 explanations per minute to prevent LLM quota exhaustion.
 */
export async function checkExplanationLimit(key: string): Promise<{
	allowed: boolean;
	error?: string;
}> {
	try {
		...
		const recent = await db
			...
			.where(
				and(
					...
					eq(rateLimits.action, "explanation"),
					gte(rateLimits.createdAt, oneMinuteAgo),
				),
			);
		...
		if (count >= 20) {
			return {
				allowed: false,
				error:
					"Too many explanation requests. Please wait a minute before requesting more explanations.",
			};
		}
		return { allowed: true };
	} catch (err: any) {
		console.error("[RateLimiter] Error checking explanation limit:", err);
		return { allowed: true };
	}
}

/**
 * Record an explanation request event.
 */
export async function recordExplanation(key: string) {
	try {
		...
		console.warn("[RateLimiter] Failed to record explanation:", err);
	}
}
