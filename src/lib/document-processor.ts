...
export async function validateFileMagicBytes(
	...
	if (expectedType === "pdf") {
		...
		if (!isPdf) {
			return {
				valid: false,
				error: "Invalid PDF format: file header does not match %PDF.",
			};
		}
		return { valid: true };
	}
	...
	try {
		...
		if (expectedType === "docx") {
			...
			if (!hasDocument) {
				return {
					valid: false,
					error: "Corrupted DOCX file: missing word/document.xml",
				};
			}
			...
			if (!hasPresentation) {
				return {
					valid: false,
					error: "Corrupted PPTX file: missing ppt presentation structures",
				};
			}
		}
		return { valid: true };
		...
		return {
			valid: false,
			error: `Corrupted ${expectedType.toUpperCase()} file: ${err.message}`,
		};
	}
}
...
/**
 * Unescapes common XML entities into standard characters.
 * Single-pass regex replacer avoids double-unescaping vulnerabilities.
 */
			...
			switch (entity) {
				...
				default:
					if (entity.startsWith("#x") || entity.startsWith("#X")) {
						...
						return !Number.isNaN(code) ? String.fromCodePoint(code) : match;
					}
...
function parseDocxXml(xml: string): string {
	...
	while ((pMatch = pRegex.exec(xml)) !== null) {
		...
		const headingLevelMatch = /<w:pStyle[^>]*w:val="Heading(\d)"/i.exec(
			pContent,
		);
		const headingLevel = headingLevelMatch ? headingLevelMatch[1] : null;

		// Extract text, line breaks (<w:br/>, <w:cr/>), and tabs (<w:tab/>)
		const runRegex =
			/<w:t(?:\s[^>]*)?>([^<]*)<\/w:t>|<w:(?:br|cr)(?:\s[^>]*)?\/>|<w:tab(?:\s[^>]*)?\/>/g;
		let text = "";
		let runMatch: RegExpExecArray | null;
		while ((runMatch = runRegex.exec(pContent)) !== null) {
			if (runMatch[1] !== undefined) {
				text += runMatch[1];
			} else if (runMatch[0].includes("tab")) {
				text += "\t";
			} else {
				text += "\n";
			}
...
function parsePptxSlideXml(xml: string): string {
	...
	while ((pMatch = pRegex.exec(xml)) !== null) {
		const pContent = pMatch[1];
		// Text elements and line breaks in PowerPoint
		const runRegex = /<a:t(?:\s[^>]*)?>([^<]*)<\/a:t>|<a:br(?:\s[^>]*)?\/>/g;
		let pText = "";
		let runMatch: RegExpExecArray | null;
		while ((runMatch = runRegex.exec(pContent)) !== null) {
			if (runMatch[1] !== undefined) {
		...
		const trimmed = unescapeXml(pText).trim();
		if (trimmed) {
			lines.push(trimmed);
		}
	}
...
export async function processDocumentQueue(
	...
	for (const message of batch.messages) {
		...
		try {
			...
			const bucket =
				workerEnv?.DOCUMENTS_BUCKET || (globalThis as any).DOCUMENTS_BUCKET;
			if (!bucket) {
				throw new Error(
					"DOCUMENTS_BUCKET binding is missing from worker environment.",
				);
			}
